...
- **Service identity middleware (`server.py`):** LM Studio's dev server answers unknown paths with **"200 anyway" + an error body**, so a bare 200 proves nothing. Every proxy response is **shape-validated** against the broker contract (`_looks_like` + `_QUEUE/_METRICS/_SPILL_SHAPE_KEYS`); a wrong-shaped 200 returns `502 {reachable: true, compatible: false}` — never rendered as data, and a spill PUT echo that isn't spill-shaped means **the write did not take**. `GET /api/local/status` fingerprints what is actually listening (`lane-broker` via /queue shape, else `lmstudio` /api/v0/models, `vllm` /version, `ollama` /api/version, `openai-compatible` /v1/models, else unknown/offline), cached 30s (`_detect_cache`). The drawer shows the identity line (green dot = lane broker; red = wrong service, with a plain-English hint) and hides the panels when incompatible.
