...
- **UI:** a `Cpu` indicator in the `TopBar` (COCKPIT bar) shows live queue depth + tps when enabled; clicking opens a drawer composing `LaneQueuePanel.jsx` (queue tile + inert spill slider) and `LocalMetricsPanel.jsx` (reporting dashboard + `lifetime|24h|session` window selector). Enablement is a `localStorage` flag (`cockpit-local-enabled`).
- **Polling (`App.jsx`):** a single 3s `setInterval` fetches `/api/local/queue` + `/api/local/metrics`, **gated on `localEnabled`** so a disabled feature does zero background work. Errors are swallowed and rendered as an offline state (mirrors the WorkflowsPanel best-effort pattern).
