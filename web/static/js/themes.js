/**
 * Claude Cockpit — Theme System
 * 10 themes x 2 variants (dark/light) = 20 total color schemes.
 */
...
let _retroFontLoaded = false;

function loadRetroFont() {
    if (_retroFontLoaded) return;
    _retroFontLoaded = true;
...
function applyTheme(id) {
    const t = getTheme(id);
    if (!t) {
        console.warn(`[themes] Unknown theme id: "${id}"`);
        return;
    }
    ...
    root.setProperty("--bg-highlight", t.bgHighlight);
    root.setProperty("--fg",           t.fg);
    root.setProperty("--fg-dim",       t.fgDim);
    root.setProperty("--fg-muted",     t.fgMuted);
    root.setProperty("--accent",       t.accent);
    root.setProperty("--accent-warm",  t.accentWarm);
    ...
    root.setProperty("--hex-base",     t.hexBase);
    root.setProperty("--hex-glow",     t.hexGlow);
    ...
    // retro font
    if (t.fontFamily.includes("Press Start 2P")) {
        loadRetroFont();
    }

    // push colors into hex grid if it exists
    if (window.HexGrid && typeof window.HexGrid.updateColors === "function") {
        window.HexGrid.updateColors(t.hexBase, t.hexGlow, t.hexGlowIntensity);
    }
