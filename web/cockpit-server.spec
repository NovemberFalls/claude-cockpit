...
# Version resource: read the version out of pyproject.toml at spec time (never
# hard-code it here -- a stale version resource is worse than none, since it
# reads as authoritative in Task Manager / file Properties). This regenerates
...
_pyproject = os.path.join(os.path.dirname(root), 'pyproject.toml')
with open(_pyproject, encoding='utf-8') as _f:
    _match = re.search(r'^version\s*=\s*"([^"]+)"', _f.read(), re.M)
if not _match:
    raise RuntimeError(f'could not find version = "..." in {_pyproject}')
...
# GENERATED by cockpit-server.spec from ../pyproject.toml's [project].version
